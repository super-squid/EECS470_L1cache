...
//   TAG_BITS    = 24 (addr[31:8])
//
// ADDR_A = 0x0000_0000  index=0, tag=0
// ADDR_B = 0x0000_0100  index=0, tag=1  <- conflict (same index, different tag)
//
// Test flow:
...
module testbench_victim_cache;
    ...
    localparam [AW-1:0] ADDR_A = 32'h0000_0000;  // tag=0, index=0
    localparam [AW-1:0] ADDR_B = 32'h0000_0100;  // tag=1, index=0 (conflict)
    ...
    task print_vc;
        ...
        for (int i = 0; i < 4; i++) begin
            if (vc.vc[i].valid)
                $display("      vc[%0d] valid  tag=0x%06h", i, vc.vc[i].tag);
            else
    ...
    initial begin
        ...
        if (rdata !== 32'h03020100) begin
            $display("  FAIL: rdata mismatch"); failures++;
        end else if (!vc.vc[3].valid || vc.vc[3].tag !== 24'h0) begin
            $display("  FAIL: expected vc[3] to hold ADDR_A (tag=0x000000)"); failures++;
        end else
            $display("  PASS: ADDR_A block stored in vc[3] (tag=0x000000)");
        ...
        //         VC absorbs ADDR_B into vc[2] (next LRU victim after vc[3] is MRU).
        // ----------------------------------------------------------------
        $display("");
        ...
        if (rdata !== 32'h03020100) begin
            $display("  FAIL: rdata mismatch"); failures++;
        end else if (!vc.vc[2].valid || vc.vc[2].tag !== 24'h1) begin
            $display("  FAIL: expected vc[2] to hold ADDR_B (tag=0x000001)"); failures++;
        end else
            $display("  PASS: data from VC; ADDR_B evicted back into vc[2] (tag=0x000001)");
