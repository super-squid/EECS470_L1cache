...
module performance_analysis_direct_mapped_assoc1;
    ...
    initial begin
        ...
        $display("Cache configuration        : assoc1: CPU -> two_way_dcache(ASSOC=1) -> DRAM");
        ...
        $display("");
        $display("Set-associative effect");
        $display("ASSOC                       : 1");
        $display("Victim-cache effect          : N/A (this configuration has no victim cache)");
        $display("Prefetch effect              : N/A (this configuration has no prefetcher)");
