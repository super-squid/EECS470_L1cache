...
module performance_analysis_prefetch;
    ...
    logic [AW-1:0]            prefetch_addr;
    logic                     prefetch_issue_pulse;
    logic                     prefetch_fill_pulse;
    logic                     prefetch_hit_pulse;
    logic [31:0]              prefetch_issue_count;
    logic [31:0]              prefetch_fill_count;
    ...
    next_line_prefetcher #(
        ...
        .prefetch_issue_pulse (prefetch_issue_pulse),
        .prefetch_fill_pulse  (prefetch_fill_pulse),
        .prefetch_hit_pulse   (prefetch_hit_pulse),
        .prefetch_issue_count (prefetch_issue_count),
        .prefetch_fill_count  (prefetch_fill_count),
    ...
    initial begin
        ...
        if (!$value$plusargs("PROG=%s", program_file)) begin
            program_file = "perf_01_sequential_stream.hex";
        end
    ...
    real prefetch_hit_rate_per_l1_read;
    ...
    assign l1_req_start   = l1_pf_if.req_valid && !prev_l1_req;
    assign dram_req_start = pf_dram_if.req_valid && !prev_dram_req;
    ...
    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            if (l1_req_start && !l1_pf_if.rw) begin
            ...
            if (dram_req_start && !pf_dram_if.rw) begin
            ...
            prev_l1_req   <= l1_pf_if.req_valid;
            prev_dram_req <= pf_dram_if.req_valid;
        end
    end
    ...
    initial begin
        ...
        if (l1_block_reads > 0)
            prefetch_hit_rate_per_l1_read = 100.0 * real'(prefetch_hit_count) / real'(l1_block_reads);
        else
            prefetch_hit_rate_per_l1_read = 0.0;

        $display("");
        $display("Performance Statistics");
        $display("Cache configuration        : prefetch: CPU -> simple_dcache -> next_line_prefetcher -> DRAM");
        ...
        $display("");
        $display("Prefetch effect");
        $display("Prefetch busy at end         : %0b", prefetch_busy);
        $display("Prefetch buffer valid at end : %0b", prefetch_valid);
        $display("Prefetch buffer addr at end  : %h", prefetch_addr);
        $display("Prefetch issues              : %0d", prefetch_issue_count);
        $display("Prefetch fills               : %0d", prefetch_fill_count);
        $display("Prefetch hits                : %0d", prefetch_hit_count);
        $display("Prefetch hits / L1 reads     : %.2f%%", prefetch_hit_rate_per_l1_read);
